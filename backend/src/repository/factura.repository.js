...
function completeSuccessfulPayment({ factura_id }) {
    return new Promise((resolve, reject) => con.query(`UPDATE factura SET estado_factura_id=2 WHERE factura.id = ?`, factura_id, (error, result) => {
        if (error) reject(error)
        resolve(result)
    }))
}
function cancelPayment({ factura_id }) {
    return new Promise((resolve, reject) => con.query(`UPDATE factura SET estado_factura_id=3 WHERE factura.id = ?`, factura_id, (error, result) => {
        if (error) reject(error)
        resolve(result)
    }))
}

module.exports = { addLibro, removeLibro, add, getLibros, getPendingFacture, updateTotal, completeSuccessfulPayment, cancelPayment }